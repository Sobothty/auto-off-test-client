...
function normalizeRepositories(payload: unknown): JsonDict[] {
...
export async function GET(
  ...
  const backendResponse = await fetch(`${backendUrl}/integrations/${provider}/repositories`, {
    method: "GET",
    headers: {
  ...
  if (backendResponse.ok === false) {
    return NextResponse.json(
      { message: readMessage(payload, `Failed to load ${provider} repositories`) },
  ...
  return NextResponse.json(
    {
      provider,
      repositories: normalizeRepositories(payload),
    },
    { status: 200 },
  );
}
